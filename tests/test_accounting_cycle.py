...
from models import (
    ...
    PayrollSlip, EmployeeSalaryPayment, SubcontractorRetentionRelease, SalesTrip,
)
...
def test_problem_screens_include_required_fields(client):
    ...
    assert 'name="check_in"' in hr.get_data(as_text=True)
...
def test_sales_trip_sheet_totals_feed_driver_account(client):
    ...
    saved = client.post("/sales", data={
        "date": "2026-09-15",
        "voucher_number": "58927",
        "trip_type": "سيارة",
        "origin": "كسارة القاهرة",
        "destination": "الكورنيش",
        "distance_km": "110",
        "driver_name": "شعبان محمد",
        "tractor_number": "1412/3673",
        "cubage": "64",
        "discount": "6",
        "unit_price": "185",
        "advances": "0",
        "period_label": "بعد العيد الكبير",
        "client_name_new": "الكتيبة",
    }, follow_redirects=True)
    assert saved.status_code == 200
    ...
    with flask_app.app_context():
        ...
        assert round(balances.get(revenue.id, 0.0), 2) == -10730
        ...
        assert round(balances.get(journal.debit_account_id, 0.0), 2) == 10730
