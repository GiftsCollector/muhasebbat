...
def test_admin_amount_eye_needs_pin(client):
    with client.session_transaction() as sess:
        sess["admin_amounts_visible"] = False
    ...
    assert "adminAmountEye" in locked_body

    wrong = client.post("/admin/amounts/unlock", data={"pin": "غلط"})
    assert wrong.status_code == 403

    unlocked = client.post("/admin/amounts/unlock", data={"pin": "ابراهيم"})
    assert unlocked.status_code == 200
    assert unlocked.get_json()["ok"] is True

    visible_home = client.get("/")
    visible_body = visible_home.get_data(as_text=True)
    assert "••••" not in visible_body

    locked_again = client.post("/admin/amounts/lock")
    assert locked_again.status_code == 200
    assert "••••" in client.get("/").get_data(as_text=True)
