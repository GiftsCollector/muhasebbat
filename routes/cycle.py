...
from services.accounting import (
    ...
    sanitize_progress_item_boq, build_account_statement, purge_operating_document, sync_client_receipt_journal,
...
def register(app):
    ...
    def update_client_receipt(receipt_id):
        ...
        if amount <= 0:
            flash("يرجى إدخال مبلغ أكبر من صفر", "danger")
            return redirect(url_for("client_receipts"))
        method = request.form.get("payment_method") or receipt.payment_method or "نقدي"
        if method not in ("نقدي", "بنكي"):
            method = "نقدي"
