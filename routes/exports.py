...
def _rows_attendance(period_month):
    headers = ["التاريخ", "الموظف", "الحالة", "حضور", "انصراف", "تأخير", "استئذان", "الحفظ"]
    query = EmployeeAttendance.query
    if period_month:
        query = query.filter(EmployeeAttendance.date.like(f"{period_month}%"))
    rows = [
        [item.date, item.employee.display_name if item.employee else "", item.status, item.check_in or "", item.check_out or "", item.delay_minutes or 0, item.permission_hours or 0, item.record_status or "مرحل"]
