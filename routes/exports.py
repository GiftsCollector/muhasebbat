...
from services.authz import filter_hidden_treasuries, filter_visible_journals, first_allowed_endpoint, user_can
...
def _rows_journal():
    ...
    rows = []
    for item in filter_visible_journals(JournalEntry.query.order_by(JournalEntry.date.desc(), JournalEntry.id.desc()).all()):
        rows.append([
            item.display_number,
            item.date or "",
            item.description or "",
...
def _rows_accounts():
    ...
    rows = [
        ...
        for item in filter_hidden_treasuries(ChartOfAccount.query.order_by(ChartOfAccount.category, ChartOfAccount.code).all())
    ]
...
def _rows_attendance(period_month):
    from routes.hr import _build_attendance_sheet

    month = (period_month or "").strip()
    employees = Employee.query.filter_by(is_active=True).order_by(Employee.name).all()
    if not employees:
        employees = Employee.query.order_by(Employee.name).all()
    headers = ["م", "الموظف", "الكود", "الحالة", "من", "إلى", "حضور", "إجازة", "غياب", "إجمالي الأيام", "الحفظ", "ملاحظة"]
    rows = []
    if month:
        for index, row in enumerate(_build_attendance_sheet(employees, month), start=1):
            rows.append([
                index,
                row["employee"].display_name,
                row["employee"].display_code,
                row["status"] or "",
                row["from_date"] or "",
                row["to_date"] or "",
                row["work_days"],
                ...
                row["absent_days"],
                row["total_days"],
                row["record_status"] or "",
                row["notes"] or "",
            ])
        rows.append(["", "", "", "", "", "", "", "", "", "", "", ""])
        rows.append(["سجل الأيام", "التاريخ", "الموظف", "الحالة", "حضور", "انصراف", "الحفظ", "ملاحظة", "", "", "", ""])
        query = EmployeeAttendance.query.filter(EmployeeAttendance.date.like(f"{month}%"))
        for item in query.order_by(EmployeeAttendance.date, EmployeeAttendance.id).all():
            rows.append([
                "",
                item.date or "",
                item.employee.display_name if item.employee else "",
                item.status or "",
                item.check_in or "",
                item.check_out or "",
                item.record_status or "مرحل",
                item.notes or "",
                "",
                "",
                "",
    ...
    return "كشف الحضور", headers, rows
