...
from models import (
    BOQItem, ChartOfAccount, ClientReceipt, CustodySettlement, DocumentAttachment,
    DriverCompensationEntry, EmployeeSalaryPayment, InventoryTransaction, JournalEntry,
    ProgressPayment, ProgressPaymentItem, Project, PurchaseOrder, SalesTrip, Subcontractor,
    SubcontractorPayment, SubcontractorRetentionRelease, Supplier, SupplierPayment, db,
)
from services.accounting import (
    ENTITY_KIND_OPTIONS, EXPENSE_CLASS_OPTIONS, PeriodClosedError, UNIT_OPTIONS,
    SUB_PAY_KIND_ADVANCE, SUB_PAY_KIND_SETTLEMENT, as_float, as_int, assert_period_open,
    build_account_balances, build_account_statement,
    build_project_cost_breakdown, build_subcontractor_statement, executed_quantity_index,
    format_grouped_number, generate_progress_payment_number, get_account_by_code,
    account_code_taken, get_custody_owner_type, get_subcontractor_outstanding_advances,
    get_subcontractor_outstanding_retention, get_treasury_balance,
    is_operational_treasury_account, is_treasury_account, next_account_code, normalize_payment_method,
    next_account_codes_by_category,
    find_client_account, list_client_names, max_advance_deduction, parse_progress_payment_items, percentage_or_amount,
    previous_executed_quantity, progress_net_remaining, purchase_order_is_received, recalculate_progress_payment, resolve_client_name,
    sanitize_progress_item_boq, subcontractor_progress_query, sync_journal_related_accounts,
    sync_posted_journals_to_documents, sync_progress_payment_journals,
    sync_retention_release_journal, sync_subcontractor_payment_journal,
)
from services.immutability import (
    ACCOUNT_DELETE_MESSAGE, OPENING_BALANCE_LOCK_MESSAGE, PARTY_DELETE_MESSAGE,
    account_has_any_journals, account_has_posted_moves, account_ids_with_posted_moves,
    delete_unused_account,
)
from services.authz import filter_hidden_treasuries, user_can_see_account


def build_driver_trip_summary(account, from_date=None, to_date=None):
    """لحسابات السواقين: يجمع نقلات شيت المبيعات المرتبطة بنفس اسم السائق ليظهروا في كشف حسابه."""
    if get_custody_owner_type(account) != "سائق":
        return None
    driver_name = (account.name or "").split("سائق - ", 1)[-1].strip()
    if not driver_name:
        return None
    query = SalesTrip.query.filter(SalesTrip.driver_name == driver_name)
    if from_date:
        query = query.filter(SalesTrip.date >= from_date)
    if to_date:
        query = query.filter(SalesTrip.date <= to_date)
    trips = query.order_by(SalesTrip.date.desc(), SalesTrip.id.desc()).all()
    if not trips:
        return {
            "driver_name": driver_name, "trips": [], "count": 0,
            "distance_total": 0.0, "cubage_total": 0.0, "amount_total": 0.0,
        }
    return {
        "driver_name": driver_name,
        "trips": trips,
        "count": len(trips),
        "distance_total": round(sum(as_float(t.distance_km) for t in trips), 2),
        "cubage_total": round(sum(as_float(t.net_quantity) for t in trips), 2),
        "amount_total": round(sum(as_float(t.total_amount) for t in trips), 2),
    }
...
def register(app):
    ...
    def accounts():
        ...
        if request.method == "POST":
            ...
            if not name:
                flash("يرجى إدخال اسم الحساب", "danger")
                return redirect(url_for("accounts"))
            category = request.form.get("category")
            code = (request.form.get("code") or "").strip()
            if code:
                taken = account_code_taken(code)
                if taken:
                    flash(
                        f"الكود {code} مسجّل على الحساب {taken.name}. غيّره أو اتركه فارغًا ليُكتب تلقائيًا.",
                        "danger",
                    )
                    return redirect(url_for("accounts"))
            else:
                code = next_account_code(category)
            ...
            try:
                ...
                return redirect(url_for("accounts"))
            flash(f"تم إضافة الحساب {account.code} بنجاح", "success")
            return redirect(url_for("accounts"))

        charts = filter_hidden_treasuries(ChartOfAccount.query.order_by(ChartOfAccount.category, ChartOfAccount.code).all())
        ...
        return render_template(
            ...
            next_account_codes=next_account_codes_by_category(category_options),
        )
    ...
    def update_account(account_id):
        ...
        if not user_can_see_account(account):
            flash("ليست لديك صلاحية عرض هذه الخزنة أو حركتها", "danger")
            return redirect(url_for("accounts"))
        name = (request.form.get("name") or "").strip()
        if not name:
            flash("يرجى إدخال اسم الحساب", "danger")
        ...
        code = (request.form.get("code") or "").strip()
        if code:
            taken = account_code_taken(code, exclude_id=account.id)
            if taken:
                flash(
                    f"الكود {code} مسجّل على الحساب {taken.name}. غيّره أو امسحه ليُكتب تلقائيًا.",
                    "danger",
                )
                return redirect(url_for("accounts"))
        else:
            code = next_account_code(category, exclude_id=account.id)
    ...
    def delete_account(account_id):
        ...
        if not user_can_see_account(account):
            ...
            return redirect(url_for("accounts"))
        referenced = (
            account_has_any_journals(account.id)
            or SubcontractorPayment.query.filter_by(treasury_account_id=account.id).first()
            or SupplierPayment.query.filter_by(treasury_account_id=account.id).first()
            or ClientReceipt.query.filter_by(treasury_account_id=account.id).first()
            or EmployeeSalaryPayment.query.filter_by(treasury_account_id=account.id).first()
            or DriverCompensationEntry.query.filter_by(treasury_account_id=account.id).first()
            or CustodySettlement.query.filter(
        ...
        try:
            ...
            flash(ACCOUNT_DELETE_MESSAGE, "danger")
            return redirect(url_for("accounts"))
        flash(f"تم حذف الحساب {code}", "success")
        return redirect(url_for("accounts"))
    ...
    def account_statement(account_id):
        ...
        driver_trips = build_driver_trip_summary(account, from_date, to_date)
        return render_template(
    ...
    @app.route("/accounts/lookup")
    def account_lookup():
        account_id = as_int(request.args.get("account_id"))
        if account_id:
            ...
            if account:
                ...
                return redirect(url_for("account_statement", account_id=account.id))
            flash("الحساب غير موجود", "warning")
            return redirect(url_for("index"))
        query = (request.args.get("q") or "").strip()
        if not query:
            flash("اختر حسابًا من القائمة", "warning")
            return redirect(url_for("index"))
        folded = query.casefold()
        matches = [
            account for account in ChartOfAccount.query.order_by(ChartOfAccount.code, ChartOfAccount.name).all()
            if user_can_see_account(account) and (
                folded in (account.name or "").casefold() or folded in (account.code or "").casefold()
            )
        ]
        if len(matches) == 1:
            return redirect(url_for("account_statement", account_id=matches[0].id))
        if not matches:
            flash(f"لا يوجد حساب يطابق «{query}»", "warning")
            return redirect(url_for("index"))
        return redirect(url_for("accounts", q=query))
    ...
    def delete_project(project_id):
        from models import Estimation, LaborEntry, Equipment
        project = Project.query.get_or_404(project_id)
        related = (
            ProgressPayment.query.filter_by(project_id=project.id).first()
            or PurchaseOrder.query.filter_by(project_id=project.id).first()
            or InventoryTransaction.query.filter_by(project_id=project.id).first()
            or JournalEntry.query.filter_by(project_id=project.id).first()
            or LaborEntry.query.filter_by(project_id=project.id).first()
            or Equipment.query.filter_by(project_id=project.id).first()
            or CustodySettlement.query.filter_by(project_id=project.id).first()
            or DriverCompensationEntry.query.filter_by(project_id=project.id).first()
            or ClientReceipt.query.filter_by(project_id=project.id).first()
            or SupplierPayment.query.filter_by(project_id=project.id).first()
            or Estimation.query.filter_by(project_id=project.id).first()
        )
        if related:
            flash(PARTY_DELETE_MESSAGE, "danger")
            return redirect(url_for("projects"))
        for boq in BOQItem.query.filter_by(project_id=project.id).all():
            if (
                ProgressPaymentItem.query.filter_by(boq_item_id=boq.id).first()
                or ChartOfAccount.query.filter_by(boq_item_id=boq.id).first()
            ):
                flash(PARTY_DELETE_MESSAGE, "danger")
                return redirect(url_for("projects"))
    ...
    @app.route("/projects/<int:project_id>/boq/<int:boq_item_id>/delete", methods=["POST"])
    def delete_boq_item(project_id, boq_item_id):
        project = Project.query.get_or_404(project_id)
        boq_item = BOQItem.query.filter_by(id=boq_item_id, project_id=project.id).first_or_404()
        if (
            ProgressPaymentItem.query.filter_by(boq_item_id=boq_item.id).first()
            or ChartOfAccount.query.filter_by(boq_item_id=boq_item.id).first()
        ):
            flash(PARTY_DELETE_MESSAGE, "danger")
            return redirect(url_for("project_detail", project_id=project.id))
        ...
        try:
            ...
            flash(PARTY_DELETE_MESSAGE, "danger")
            return redirect(url_for("project_detail", project_id=project.id))
        flash("تم حذف بند الأعمال", "success")
        return redirect(url_for("project_detail", project_id=project.id))
    ...
    def progress_payments():
        ...
        treasury_accounts = filter_hidden_treasuries(
            [account for account in accounts if is_operational_treasury_account(account)]
        )
        ...
        if request.method == "POST":
            ...
            item_lines = sanitize_progress_item_boq(parse_progress_payment_items(request.form), project_id)
            if not item_lines:
            ...
            discount_insurance = percentage_or_amount(
                retention_percentage, request.form.get("discount_insurance"), works_total
            )
            tax_value = percentage_or_amount(tax_percentage, request.form.get("tax"), works_total)
            discount_value = percentage_or_amount(
                discount_percentage, request.form.get("discount_value"), works_total
            )
            ...
            deductions_total = round(
                discount_insurance + tax_value + discount_value + penalties + other_deductions, 2
            )
            outstanding_advances = get_subcontractor_outstanding_advances(subcontractor_id)
            cap = max_advance_deduction(works_total, deductions_total, outstanding_advances)
            advance_deduction = as_float(request.form.get("advance_deduction"))
            if advance_deduction > cap + 0.01:
                flash(
                    f"أقصى خصم للدفعات تحت الحساب هو {format_grouped_number(cap)} "
                    f"(المتاح {format_grouped_number(outstanding_advances)})",
                    "danger",
                )
                return redirect(url_for("progress_payments"))
            advance_deduction = min(max(0.0, advance_deduction), cap)
            ...
            payment = ProgressPayment(
                ...
                discount_insurance=discount_insurance,
                tax=tax_value,
            ...
            db.session.flush()
            recalculate_progress_payment(payment, outstanding_advances=outstanding_advances)
            try:
        ...
        cash_payments = SubcontractorPayment.query.order_by(SubcontractorPayment.id.desc()).all()
        outstanding_advances_map = {
            item.id: get_subcontractor_outstanding_advances(item.id) for item in subcontractors
        }
        outstanding_retention_map = {
            item.id: get_subcontractor_outstanding_retention(item.id) for item in subcontractors
        }
        remaining_map = {item.id: progress_net_remaining(item) for item in payments}
        totals = {
            ...
            "remaining": round(sum(remaining_map.values()), 2),
        }
        return render_template(
            ...
            outstanding_retention_map=outstanding_retention_map,
            remaining_map=remaining_map,
            executed_qty_rows=executed_quantity_index(),
            totals=totals,
    ...
    def update_progress_payment(payment_id):
        ...
        project_id = as_int(request.form.get("project_id")) or payment.project_id
        item_lines = sanitize_progress_item_boq(parse_progress_payment_items(request.form), project_id)
        if not item_lines:
            flash("يرجى إدخال بند واحد على الأقل بكمية وسعر فئة أكبر من صفر", "danger")
            return redirect(url_for("progress_payment_detail", payment_id=payment.id))
        ...
        if "subcontractor_id" in request.form:
            payment.subcontractor_id = as_int(request.form.get("subcontractor_id")) or payment.subcontractor_id
        ...
        payment.discount_percentage = as_float(request.form.get("discount_percentage"))
        payment.discount_insurance = as_float(request.form.get("discount_insurance"))
        payment.tax = as_float(request.form.get("tax"))
        payment.discount_value = as_float(request.form.get("discount_value"))
        ...
        works_total = round(sum(as_float(line["value"]) for line in item_lines), 2)
        discount_insurance = percentage_or_amount(
            payment.retention_percentage, payment.discount_insurance, works_total
        )
        tax_value = percentage_or_amount(payment.tax_percentage, payment.tax, works_total)
        discount_value = percentage_or_amount(
            payment.discount_percentage, payment.discount_value, works_total
        )
        deductions_total = round(
            discount_insurance + tax_value + discount_value
            + as_float(payment.penalties) + as_float(payment.other_deductions), 2
        )
        outstanding_advances = get_subcontractor_outstanding_advances(
            payment.subcontractor_id, exclude_payment_id=payment.id
        )
        cap = max_advance_deduction(works_total, deductions_total, outstanding_advances)
        advance_deduction = as_float(request.form.get("advance_deduction"))
        if advance_deduction > cap + 0.01:
            ...
            return redirect(url_for("progress_payment_detail", payment_id=payment.id))
        payment.advance_deduction = min(max(0.0, advance_deduction), cap)
        payment.discount_insurance = discount_insurance
        payment.tax = tax_value
    ...
    def progress_payment_detail(payment_id):
        ...
        for item in items:
            item.previous_qty = previous_executed_quantity(
                payment.project_id,
                payment.subcontractor_id,
                item.boq_item_id,
                item.description,
                item.unit,
                payment.id,
        ...
        remaining = progress_net_remaining(payment)
        outstanding_advances = get_subcontractor_outstanding_advances(
            payment.subcontractor_id, exclude_payment_id=payment.id
        )
        outstanding_retention = get_subcontractor_outstanding_retention(payment.subcontractor_id)
        linked_cash = SubcontractorPayment.query.filter_by(progress_payment_id=payment.id)\
            .order_by(SubcontractorPayment.date.asc(), SubcontractorPayment.id.asc()).all()
        client_account = None
        if not payment.subcontractor_id and payment.project and payment.project.client_name:
            client_account = find_client_account(payment.project.client_name)
        return render_template(
            ...
            outstanding_retention=outstanding_retention,
            linked_cash=linked_cash,
            executed_qty_rows=executed_quantity_index(exclude_payment_id=payment.id),
            today_date=date.today().isoformat(),
    ...
    def create_subcontractor_payment():
        """تسجيل دفعة نقدية/بنكية لمقاول الباطن: تحت الحساب أو صرف صافي مستخلص."""
        sync_journal_related_accounts()
        subcontractor_id = as_int(request.form.get("subcontractor_id"))
        ...
        payment_kind = (request.form.get("payment_kind") or SUB_PAY_KIND_ADVANCE).strip()
        if payment_kind not in (SUB_PAY_KIND_ADVANCE, SUB_PAY_KIND_SETTLEMENT):
            payment_kind = SUB_PAY_KIND_ADVANCE

        def bounce():
            if redirect_target == "subcontractor_statement" and subcontractor_id:
                return redirect(url_for("subcontractor_statement", subcontractor_id=subcontractor_id))
            if redirect_target == "progress_payment_detail" and progress_payment_id:
                return redirect(url_for("progress_payment_detail", payment_id=progress_payment_id))
            return redirect(url_for("progress_payments"))
        ...
        ipc = None
        if payment_kind == SUB_PAY_KIND_SETTLEMENT:
            if not progress_payment_id:
                flash("صرف صافي المستخلص يتطلب اختيار المستخلص", "danger")
                return bounce()
            ipc = ProgressPayment.query.get(progress_payment_id)
            if not ipc or ipc.subcontractor_id != subcontractor_id:
                flash("المستخلص غير مرتبط بهذا المقاول", "danger")
                return bounce()
            remaining = progress_net_remaining(ipc)
            if amount > remaining + 0.01:
                flash(
                    f"المتبقي لصرف هذا المستخلص هو {format_grouped_number(remaining)} فقط",
                    "danger",
                )
                return bounce()
        else:
            progress_payment_id = None
        ...
        if not treasury_account_id:
            ...
            if fallback and user_can_see_account(fallback):
                treasury_account_id = fallback.id
            else:
                visible = filter_hidden_treasuries(
                    [account for account in ChartOfAccount.query.all() if is_operational_treasury_account(account)]
                )
                treasury_account_id = visible[0].id if visible else None
        if treasury_account_id and not user_can_see_account(treasury_account_id):
            flash("ليست لديك صلاحية استخدام هذه الخزنة", "danger")
            return bounce()

        advance = SubcontractorPayment(
            subcontractor_id=subcontractor_id,
            project_id=as_int(request.form.get("project_id")) or (ipc.project_id if ipc else None),
            progress_payment_id=progress_payment_id,
        ...
        try:
            ...
            return bounce()
        if payment_kind == SUB_PAY_KIND_SETTLEMENT:
            flash(f"تم صرف صافي المستخلص بقيمة {format_grouped_number(amount)}", "success")
        else:
            flash(f"تم تسجيل دفعة تحت الحساب بقيمة {format_grouped_number(amount)}", "success")
        return bounce()


    @app.route("/retention_releases", methods=["POST"])
    def create_retention_release():
        ...
        if not subcontractor_id:
            ...
            return bounce()
        if amount <= 0:
            flash("يرجى إدخال مبلغ إفراج أكبر من صفر", "danger")
            return bounce()
        outstanding = get_subcontractor_outstanding_retention(subcontractor_id)
        if amount > outstanding + 0.01:
            flash(
                f"الضمان المحتجز المتاح للإفراج هو {format_grouped_number(outstanding)} فقط",
                "danger",
            )
            return bounce()
        release_date = request.form.get("date") or date.today().isoformat()
        try:
            assert_period_open(release_date)
        except PeriodClosedError as exc:
            flash(str(exc), "danger")
            return bounce()
        release = SubcontractorRetentionRelease(
            subcontractor_id=subcontractor_id,
            project_id=as_int(request.form.get("project_id")),
            date=release_date,
            amount=amount,
        ...
        db.session.commit()
        try:
            sync_retention_release_journal(release)
        except PeriodClosedError as exc:
            flash(str(exc), "danger")
            return bounce()
        flash(f"تم إفراج ضمان بقيمة {format_grouped_number(amount)}", "success")
        return bounce()
    ...
    def subcontractor_statement(subcontractor_id):
        ...
        return render_template(
            ...
            treasury_accounts=treasury_accounts,
            projects=projects,
            outstanding_retention=get_subcontractor_outstanding_retention(subcontractor.id),
            remaining_map={item.id: progress_net_remaining(item) for item in statement["payments"]},
            today_date=date.today().isoformat(),
        )
    ...
    def delete_subcontractor(subcontractor_id):
        item = Subcontractor.query.get_or_404(subcontractor_id)
        if (
            ProgressPayment.query.filter_by(subcontractor_id=item.id).first()
            or SubcontractorPayment.query.filter_by(subcontractor_id=item.id).first()
            or SubcontractorRetentionRelease.query.filter_by(subcontractor_id=item.id).first()
        ):
            flash(PARTY_DELETE_MESSAGE, "danger")
            return redirect(url_for("subcontractors"))
        account = get_account_by_code(f"SUB-{item.id:04d}")
        ...
        try:
            ...
            flash(PARTY_DELETE_MESSAGE, "danger")
            return redirect(url_for("subcontractors"))
        flash(f"تم حذف جهة التعامل {name}", "success")
        return redirect(url_for("subcontractors"))
    ...
    def supplier_statement(supplier_id):
        ...
        total_orders_value = sum(
            as_float(item.total_value) for item in supplier_orders if purchase_order_is_received(item)
        )
    ...
    def delete_supplier(supplier_id):
        ...
        if (
            ...
            or SupplierPayment.query.filter_by(supplier_id=item.id).first()
            or InventoryTransaction.query.filter_by(supplier_id=item.id).first()
        ):
            flash(PARTY_DELETE_MESSAGE, "danger")
            return redirect(url_for("suppliers"))
        account = get_account_by_code(f"SUP-{item.id:04d}")
        if account and account_has_any_journals(account.id):
            flash(PARTY_DELETE_MESSAGE, "danger")
            return redirect(url_for("suppliers"))
        name = item.name
        ...
        try:
            ...
            flash(PARTY_DELETE_MESSAGE, "danger")
            return redirect(url_for("suppliers"))
        flash(f"تم حذف المورد {name}", "success")
        return redirect(url_for("suppliers"))


    @app.route("/import-workbook", methods=["GET", "POST"])
    def import_workbook():
        if request.method == "POST":
            file_storage = request.files.get("workbook")
            if not file_storage or not file_storage.filename:
                flash("ارفع ملف إكسل من النموذج", "danger")
                return redirect(url_for("import_workbook"))
            filename = (file_storage.filename or "").lower()
            if not filename.endswith(".xlsx"):
                flash("استخدم ملف Excel بامتداد xlsx", "danger")
                return redirect(url_for("import_workbook"))
            from services.workbook import import_workbook_file
            try:
                summary = import_workbook_file(file_storage)
            except Exception:
                db.session.rollback()
                flash("تعذر قراءة الملف. نزّل النموذج واملأه كما هو دون تغيير أسماء الأوراق أو العناوين.", "danger")
                return redirect(url_for("import_workbook"))
            parts = [
                f"حسابات جديدة {summary['accounts_added']}",
                f"حسابات محدّثة {summary['accounts_updated']}",
                f"موردون {summary['suppliers_added']}",
                f"مقاولون {summary['subs_added']}",
                f"موظفون {summary['employees_added']}",
            ]
            flash("تم النقل: " + " — ".join(parts), "success")
            for note in summary["skipped"][:8]:
                flash(note, "warning")
            return redirect(url_for("import_workbook"))
        return render_template("import_workbook.html")


    @app.route("/import-workbook/template")
    def download_import_template():
        from services.workbook import build_import_workbook, workbook_bytes
        buffer = workbook_bytes(build_import_workbook(include_existing=False))
        return send_file(
            buffer,
            as_attachment=True,
            download_name="نموذج-نقل-الشغل.xlsx",
            mimetype="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        )


    @app.route("/import-workbook/current")
    def download_current_workbook():
        from services.workbook import build_import_workbook, workbook_bytes
        buffer = workbook_bytes(build_import_workbook(include_existing=True))
        return send_file(
            buffer,
            as_attachment=True,
            download_name="بيانات-الحالية.xlsx",
            mimetype="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        )
