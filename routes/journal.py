...
from services.authz import filter_hidden_treasuries, filter_visible_journals, touches_hidden_treasury, user_can
...
def register(app):
    ...
    def journal():
        ...
        if request.method == "POST":
            ...
            for idx in range(max_len):
                ...
                if debit_id not in valid_account_ids or credit_id not in valid_account_ids:
                    ...
                    return redirect(url_for("journal"))
                if touches_hidden_treasury(debit_id, credit_id):
                    flash("ليست لديك صلاحية استخدام هذه الخزنة في القيود", "danger")
                    return redirect(url_for("journal"))
                if amount <= 0:
        ...
        entries = filter_visible_journals(query.order_by(JournalEntry.date.desc(), JournalEntry.id.desc()).all())
    ...
    def update_journal_entry(entry_id):
        ...
        if not ChartOfAccount.query.get(debit_account_id) or not ChartOfAccount.query.get(credit_account_id):
            ...
            return redirect(url_for("journal_entry_detail", entry_id=entry.id))
        if touches_hidden_treasury(entry.debit_account_id, entry.credit_account_id, debit_account_id, credit_account_id):
            flash("ليست لديك صلاحية استخدام هذه الخزنة في القيود", "danger")
            return redirect(url_for("journal"))
        if entry_number:
    ...
    def copy_journal_entry(entry_id):
        ...
        if not filter_visible_journals([source]):
            flash("ليست لديك صلاحية عرض هذا القيد لأنه يخص خزنة محجوبة", "danger")
            return redirect(url_for("journal"))
    ...
    def reverse_journal_entry(entry_id):
        ...
        try:
            assert_period_open(source.date)
            assert_period_open(reversed_entry.date)
        except PeriodClosedError as exc:
            flash(str(exc), "danger")
