...
from services.accounting import (
    ...
    get_account_by_code, get_or_create_employee_account, is_treasury_account,
...
def _next_employee_code():
    used = set()
    for emp in Employee.query.with_entities(Employee.id, Employee.code).all():
        if emp.code and emp.code.strip():
            used.add(emp.code.strip().upper())
        used.add(f"EMP-{emp.id:04d}")
    index = 1
    while f"EMP-{index:04d}" in used:
        index += 1
    return f"EMP-{index:04d}"


def _employee_code_taken(code, exclude_id=None):
    raw = (code or "").strip()
    if not raw:
        return None
...
def register(app):
    ...
    def employees():
        ...
        if request.method == "POST":
            ...
            if code:
                taken = _employee_code_taken(code)
                if taken:
                    flash(
        ...
        return render_template(
            ...
            remaining=remaining,
            next_employee_code=_next_employee_code(),
        )
    ...
    def update_employee(employee_id):
        ...
        if not name:
            ...
            return redirect(url_for("employees"))
        item.name = name
        new_code = (request.form.get("code") or "").strip() or item.code
        if new_code:
            taken = _employee_code_taken(new_code, exclude_id=item.id)
            if taken:
                flash(
                    f"الكود {new_code} مسجّل على الموظف {taken.display_name}. غيّره قبل الحفظ.",
                    "danger",
                )
                return redirect(url_for("employees"))
        item.code = new_code or _next_employee_code()
