...
def _attendance_stats(period_month):
    ...
    for row in rows:
        if (getattr(row, "record_status", None) or "مرحل") == "مسودة":
            continue
...
def _build_attendance_sheet(employees, period_month):
    ...
    for employee in employees:
        ...
        sheet.append({
            ...
            "record_status": (last.record_status if last and getattr(last, "record_status", None) else ""),
        })
    return sheet
...
def _save_attendance_sheet(period_month, post=False):
    ...
    if post:
        flash(
            f"تم ترحيل {saved_days} يوم حضور وتحديث {payroll_updated} كشف مرتب بأيام العمل الفعلية.",
            "success",
        )
    else:
        flash(f"تم حفظ {saved_days} يوم حضور كمسودة دون التأثير على كشف المرتبات.", "success")
    return redirect(url_for("hr_module", period_month=period_month, tab="attendance"))
