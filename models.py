...
class SalesTrip(ActorStamp, db.Model):
    ...
    cubage = db.Column(db.Float, default=0)
    discount = db.Column(db.Float, default=0)
    net_quantity = db.Column(db.Float, default=0)
    unit_price = db.Column(db.Float, default=0)
    ...
    def recalculate(self):
        cubage = self.cubage or 0
        discount = self.discount or 0
        price = self.unit_price or 0
        advances = self.advances or 0
        net = max(cubage - discount, 0)
...
class EmployeeAttendance(ActorStamp, db.Model):
    """سجل يومي: حضور، انصراف، تأخير، استئذان، إجازة."""
    ...
    permission_hours = db.Column(db.Float, default=0)
    notes = db.Column(db.Text, nullable=True)
...
class PayrollSlip(ActorStamp, db.Model):
    ...
    vacation_days = db.Column(db.Float, default=0)
    basic_salary = db.Column(db.Float, default=0)
    daily_rate = db.Column(db.Float, default=0)
    earned_salary = db.Column(db.Float, default=0)
    transport = db.Column(db.Float, default=0)
    overtime = db.Column(db.Float, default=0)
    incentives = db.Column(db.Float, default=0)
    delay_deduction = db.Column(db.Float, default=0)
    permission_deduction = db.Column(db.Float, default=0)
    other_deductions = db.Column(db.Float, default=0)
    advances = db.Column(db.Float, default=0)
    notes = db.Column(db.Text, nullable=True)
    ...
    def daily_rate_value(self):
        stored = self.daily_rate or 0
        if stored:
            return round(stored, 2)
        basic = self.basic_salary or 0
        return round(basic / 30.0, 2) if basic else 0.0

    @property
    def earned_salary_value(self):
        stored = self.earned_salary or 0
        if stored:
            return round(stored, 2)
        basic = self.basic_salary or 0
        days = max(self.work_days or 0, 0)
        if basic:
            return round(basic * days / 30.0, 2)
        rate = self.daily_rate or 0
        if rate:
