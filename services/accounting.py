...
def _assert_receipt_reduction_allowed(order, wanted):
    for row in _po_auto_inventory_rows(order.id):
        key = normalize_name(row.material_name).casefold()
        old_qty = as_float(row.quantity)
        new_qty = as_float(wanted.get(key, {}).get("quantity", 0))
...
def sync_purchase_order_to_inventory(order):
    ...
    for line in lines:
        ...
        wanted[key] = {
            ...
            "notes": (
                f"{marker}:{key} - إضافة تلقائية من أمر شراء {order.document_number} - "
                f"المورد {(order.supplier.name if order.supplier else 'بدون مورد')}"
            ),
        }

    _assert_receipt_reduction_allowed(order, wanted)
