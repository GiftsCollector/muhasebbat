...
EMPLOYEE_HEADERS = ["الاسم", "الميزة", "الوظيفة", "البلد", "الموقع", "المرتب الأساسي", "الاتصال", "ملاحظات"]
...
def excel_download(filename, headers, rows, title=None):
    workbook = Workbook()
    default_sheet = workbook.active
    workbook.remove(default_sheet)
    _sheet(workbook, title or "بيانات", headers, rows)
    stamp = filename if filename.endswith(".xlsx") else f"{filename}.xlsx"
    return send_file(
...
def _import_employees(sheet, summary):
    ...
    for row_index in range(2, sheet.max_row + 1):
        ...
        job_title = _cell_text(values.get("الوظيفة")) or tag
        hometown = _cell_text(values.get("البلد")) or None
        site = _cell_text(values.get("الموقع")) or None
