...
def costs():
    # تأكد من توفر جميع الحسابات المطلوبة
    sync_journal_related_accounts()
    ...
    if request.method == "POST":
        ...
        if is_batch:
            ...
            project_ids = request.form.getlist("batch_project_id")
            boq_item_ids = request.form.getlist("batch_boq_item_id")
            cost_types = request.form.getlist("batch_cost_type")
            amounts = request.form.getlist("batch_amount")
            descriptions = request.form.getlist("batch_description")
            cost_centers = request.form.getlist("batch_cost_center")
            
            success_count = 0
            error_count = 0
            created_entries = []  # لتتبع الإدخالات المنشأة
            
            # Process each row
            for i in range(len(project_ids)):
                project_id = as_int(project_ids[i])
                boq_item_id = as_int(boq_item_ids[i]) if i < len(boq_item_ids) else None
                cost_type = cost_types[i] if i < len(cost_types) else "مباشرة"
                amount = as_float(amounts[i] if i < len(amounts) else 0)
                ...
                try:
                    ...
                    created_entries.append(cost_entry)
                    success_count += 1
                except Exception as e:
                    error_count += 1
                    print(f"Error adding cost entry: {e}")
            
            # Commit كل التغييرات
            db.session.commit()
            
            # إنشاء قيود محاسبية تلقائية للتكاليف
            for cost_entry in created_entries:
                try:
                    amount = as_float(cost_entry.amount)
                    if amount > 0:
                        # استخدم حساب المصاريف كـ debit والمشروع كـ credit
                        debit_account = ChartOfAccount.query.filter(
                            ChartOfAccount.category == "المصروفات"
                        ).first()
                        credit_account = None
                        if cost_entry.project_id:
                            credit_account = ChartOfAccount.query.filter_by(
                                code=f"PRJ-{cost_entry.project_id:04d}"
                            ).first()
                        
                        if debit_account and credit_account:
                            journal = JournalEntry(
                                date=date.today().isoformat(),
                                reference=f"CST-{cost_entry.id:06d}",
                                journal_name="المصاريف",
                                status="مرحل",
                                description=f"قيد تكلفة تلقائي - {cost_entry.description or cost_entry.cost_type}",
                                debit_account_id=debit_account.id,
                                credit_account_id=credit_account.id,
                                amount=amount,
                    ...
                    print(f"Error creating journal for cost entry {cost_entry.id}: {e}")
            
            db.session.commit()
            
            if success_count > 0:
                flash(f"تم إضافة {success_count} قيد تكلفة بنجاح + قيود محاسبية تلقائية", "success")
            if error_count > 0:
                flash(f"حدث خطأ مع {error_count} صف", "warning")
