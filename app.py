...
def _mask_admin_amounts():
    user = getattr(g, "current_user", None)
    return bool(user and user.role == ROLE_ADMIN and not session.get("admin_amounts_visible"))
...
def inject_current_user():
    ...
    return {
        ...
        "mask_amounts": _mask_admin_amounts(),
        "user_can": user_can,
...
def secretnum_filter(value):
    if _mask_admin_amounts():
        return "••••"
    return format_grouped_number(value)


app.jinja_env.filters["groupnum"] = secretnum_filter
...
ADMIN_AMOUNT_PIN = (os.getenv("ADMIN_AMOUNT_PIN") or "ابراهيم").strip()
...
def unlock_admin_amounts():
    ...
    if pin != ADMIN_AMOUNT_PIN:
        return jsonify({"ok": False, "message": "الرقم السري غير صحيح"}), 403
    session["admin_amounts_visible"] = True
...
def lock_admin_amounts():
    if g.current_user is None or g.current_user.role != ROLE_ADMIN:
        return jsonify({"ok": False}), 403
    session["admin_amounts_visible"] = False
    return jsonify({"ok": True})
