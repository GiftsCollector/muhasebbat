...
from services.authz import (
    ...
    endpoint_allowed, normalize_legacy_role, require_perm, sanitize_permissions, user_can,
)
...
def purchase_orders():
    ...
    if request.method == "POST":
        ...
        if status == "مفتوح":
            flash("تم حفظ أمر الشراء. الاستلام للمخزون يتم عند تغيير الحالة إلى مغلق أو مدفوع.", "success")
        else:
            flash("تم حفظ أمر الشراء واستلام الأصناف في المخزن مع قيد المورد.", "success")
        return redirect(url_for("purchase_orders"))
