...
def journal():
    ...
    if request.method == "POST":
        ...
        flash(
            f"تم حفظ {len(parsed_lines)} قيد بإجمالي {format_grouped_number(total_amount)}"
            f" بحالة {status}",
            "success",
        )
        return redirect(url_for("journal"))
...
def update_journal_entry(entry_id):
    ...
    entry.entry_number = (request.form.get("entry_number") or "").strip() or None
    entry.reference = (request.form.get("reference") or "").strip() or None
